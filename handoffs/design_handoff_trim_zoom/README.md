...
- **Components (von oben):**
  1. Titelblock zentriert: "Evening Wind Down" (Serif 20px), "Tara Goldstein · 19:05" (12.5px, `--sm-text-2`).
  2. Großer Readout: Label "Anfang/Ende der Wiedergabe" (`metaLbl`); Zeit in Serif **46px**,
     Farbe `--sm-accent-text`, `font-feature-settings:"tnum"`; darunter "Hörbar: a – b · dauer" (12.5px).
  3. **`MiniMap`** (nur wenn `zoomed`): Gesamtdatei-Streifen mit Fensterrahmen — siehe „Präzises Zuschneiden".
  4. **`TrimTrack`** (Kern, siehe unten), `height = 104`, gemappt über `win = [winLo, winHi]`.
  5. Zeitachse darunter: drei Spans `winLo` / aktuelle Playhead-Zeit (Sage wenn spielt) / `winHi` (11px).
  6. Zwei `ReadoutCard` (Anfang / Ende) mit kleinem Zoom-Icon, `gap:10`. Aktive Karte: Hintergrund
     `--sm-accent-dim`, Border `rgba(214,138,110,.4)`; Label `metaLbl`, Wert Serif 25px. Tippen wählt die
     Marke, setzt Playhead **und zoomt** auf sie (`frameWindow`).
  7. Chip-Reihe (flex-wrap, zentriert): im Zoom „⌕ Ganze Datei" (zoomt aus) + Snap-Chips
     „Intro überspringen · 1:30" / „Ausklang kürzen · 18:25". Stil siehe `chipStyle(smart)`.
  8. Transport: `−1s` | runder Play/Pause-Button (62px) | `+1s`. Play startet ab der **Abspielposition**.
  9. Hinweistext (kontextabhängig): Überblick „Anfang/Ende tippen = heranzoomen · oben ziehen = Abspielposition"
     / Zoom „Detailansicht — fein ziehen, ±1s für die Sekunde".
  10. Textbutton "Ganze Datei verwenden" (setzt start=0, end=TOTAL, win=[0,TOTAL]).
...
- `x ↔ Sekunde` linear über das **Fenster**: `sec = clamp((x − trackLeft)/trackWidth, 0, 1) · (winHi−winLo) + winLo`
  (im Überblick `winLo=0, winHi=TOTAL`). Siehe Abschnitt „Präzises Zuschneiden an den Rändern".
...
### Präzises Zuschneiden an den Rändern (Zoom / Minimap / Snap)  ← **wichtig**
Problem: Bei langen Dateien ist eine Sekunde nur ein Bruchteil eines Pixels breit → das Überspringen
von Intro/Ausklang (der häufigste Fall) ist an den Enden nicht präzise bedienbar. Lösung = drei
zusammenspielende Mechanismen. **Alle Spur-Interaktionen mappen über ein Fenster `[winLo, winHi]`
statt über `[0, TOTAL]`** (siehe `pctW`, `secAt`, `pxOf` in der Referenz); im Überblick ist
`win = [0, TOTAL]`.

**A) Detail-/Zoom-Ansicht.** Tippen auf die Karte *Anfang* bzw. *Ende* wählt die Marke **und** rahmt
sie in einem Fenster der Breite `ZS`:
```
ZS = min(TOTAL, max(120, round(TOTAL * 0.18)))   // Fensterbreite in s (skaliert, gedeckelt)
frameWindow(mark, "start") = [lo, lo+ZS],  lo = clamp(mark - ZS*0.25, 0, TOTAL-ZS)
frameWindow(mark, "end")   = [hi-ZS, hi],  hi = clamp(mark + ZS*0.25, ZS, TOTAL)
```
Die Marke sitzt ~25 % vom nahen Rand. Effekt: ~5× feinere Auflösung (Beispiel 19-Min-Datei: aus
~0,34 px/s werden ~2 px/s; bei 60 Min entsprechend mehr). Recentering passiert bei: Karten-Tipp,
Nudge, Snap und am **Drag-Ende** (`onScrubEnd` rahmt die freigegebene Marke neu). Während des Ziehens
bleibt das Fenster fix; reicht der Bereich nicht, hebt man kurz ab → Fenster recentert → weiter.
Die **Zeitachse** unter der Spur zeigt `winLo … winHi` (nicht `0 … TOTAL`).

**B) Minimap** (nur im Zoom sichtbar, `MiniMap`): dünner Gesamtstreifen (h 26) mit Region-Füllung,
Marken-Ticks, Playhead-Tick und einem Sage-Fensterrahmen. Pointerdown/-move darauf = Fenster
**panen** (`panWindow(center) = [clamp(center-ZS/2,0,TOTAL-ZS), …]`). Liegt eine Marke außerhalb des
Fensters, zeigt die Hauptspur statt des Griffs einen **Edge-Chip** am näheren Rand
(„Ende 19:05 ›"); Tippen wählt/rahmt diese Marke (`onPickMark`).

**C) Schnell-Vorschläge** (Snap-Chips, in allen Varianten): „Intro überspringen · `m:ss`" setzt
`start = MARKERS[0]`, „Ausklang kürzen · `m:ss`" setzt `end = MARKERS[1]`, jeweils + aktiv setzen,
Fenster rahmen, kurze Vorschau. In Produktion kommen diese Werte aus der **Stille-/Onset-Erkennung**
der echten Datei (hier Platzhalter `MARKERS = [90, 1105] s`).

**D) ±1s-Nudge** nagelt die exakte Sekunde (verschiebt die aktive Marke, recentert im Zoom, 1,4 s Vorschau).

Zoom-Steuerung im UI: `zoomed = (winHi-winLo) < TOTAL-1`. „Ganze Datei" (mit Lupe-aus-Icon) setzt
`win=[0,TOTAL]`. Karten tragen ein kleines Zoom-Icon als Hinweis.
...
- Im Sheet lokal: `start`, `end`, `active ("start"|"end")`, `head`, `playing`, `previewing`,
  `win = [winLo, winHi]` (Zoom-Fenster; Default `[0, TOTAL]`).
- Beim Öffnen: Werte aus `trim ?? {0, TOTAL}` übernehmen, `active="start"`, `head=start`, `win=[0,TOTAL]`, pausieren.
...
**Konstanten:** `TOTAL=1145`, `MINGAP=25`, `GRAB=22`, `SPLIT=0.45`, `ZS=min(TOTAL,max(120,round(TOTAL*0.18)))`, Lane-Höhe 34, Spur-Höhe 104, Minimap-Höhe 26.
