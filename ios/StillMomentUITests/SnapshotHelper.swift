...
open class Snapshot: NSObject {
    ...
    open class func setupSnapshot(_ app: XCUIApplication, waitForAnimations: Bool = true) {
        ...
        do {
            ...
            self.setLanguage(app)
            self.setLocale(app)
            self.setLaunchArguments(app)
        } catch {
    ...
    class func setLanguage(_ app: XCUIApplication) {
        ...
        do {
            ...
            self.deviceLanguage = try String(contentsOf: path, encoding: .utf8).trimmingCharacters(in: trimCharacterSet)
            app.launchArguments += ["-AppleLanguages", "(\(self.deviceLanguage))"]
        } catch {
    ...
    class func setLocale(_ app: XCUIApplication) {
        ...
        do {
            let trimCharacterSet = CharacterSet.whitespacesAndNewlines
            self.currentLocale = try String(contentsOf: path, encoding: .utf8).trimmingCharacters(in: trimCharacterSet)
        } catch {
            NSLog("Couldn't detect/set locale...")
        }

        if self.currentLocale.isEmpty, !self.deviceLanguage.isEmpty {
            self.currentLocale = Locale(identifier: self.deviceLanguage).identifier
        }

        if !self.currentLocale.isEmpty {
            app.launchArguments += ["-AppleLocale", "\"\(self.currentLocale)\""]
        }
    }
    ...
    open class func snapshot(_ name: String, timeWaitingForIdle timeout: TimeInterval = 20) {
        if timeout > 0 {
            self.waitForLoadingIndicatorToDisappear(within: timeout)
        }
        ...
        let image = XCUIDevice.shared.orientation.isLandscape ? self
        ...
        do {
            // The simulator name contains "Clone X of " inside the screenshot file when running parallelized UI Tests
            // on concurrent devices
...
private extension XCUIElementAttributes {
    var isNetworkLoadingIndicator: Bool {
        if self.hasAllowListedIdentifier { return false }
