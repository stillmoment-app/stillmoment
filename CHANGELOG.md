...
- **Player-Typografie auf Editorial-Voice umgestellt** - Der Guided-Meditation-Player traegt jetzt die "Kerzenschein 2.0"-Editorial-Sprache: der Lehrer-Name oben ist Newsreader Italic 18 in Sunrise-Akzent, der Titel darunter Newsreader Light 30 (statt vorher Semibold 28 — leichter und ruhig, lange Titel umbrechen nicht mehr in drei Zeilen), das Restzeit-Label am unteren Rand ist Geist Regular 11 mit grosszuegigem 0.2em-Tracking. Italic-Schnitte sind ab sofort orthogonal zum Weight im Typografie-System abgebildet (neue `isItalic`-Achse pro Rolle, nutzt den Newsreader16pt-Italic-Schnitt aus dem Bundle statt SwiftUIs synthetischer Italic-Variante). (Ticket: ios-048)
...
- **Buttons + Stepper-Werte auf Geist umgestellt** - Der primaere und sekundaere Button-Style (Beginnen-Knopf, Sekundaer-Aktionen) sprachen bisher noch SF Pro Rounded — sie sind jetzt Geist Medium 15pt nach Handoff-Spec (Rolle "CTA / Play"). Auch die Werte in der Timer-Idle-Liste (Vorbereitung, Gong, Intervall, Hintergrund) tragen statt SF Pro Rounded jetzt Geist Regular, konsistent mit den Labels daneben. Damit gehen alle echten UI-Texte der App durch das Typografie-System (`TypographyRole`-Rollen). (Ticket: ios-048)
- **Debug-Werkzeug "Typography Reference" (nur Debug-Build)** - Eine neue Settings → Debug → Typography Reference Seite zeigt alle 29 Typografie-Rollen Side-by-Side in Light und Dark Mode auf einer Seite. Wird fuer visuelles Tuning genutzt — spart Navigation durch die App fuer jede Rolle einzeln. In Release-Builds nicht enthalten. (Ticket: ios-048)
