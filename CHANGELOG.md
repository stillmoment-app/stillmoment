...
### Added (iOS)
- **Atemkreis-Player fuer Guided Meditations** - Der Player ist auf eine Geste reduziert: Pause/Play in der Hauptphase ist die einzige sichtbare Bedienung. Auto-Start beim Oeffnen — kein initialer Play-Tap mehr. Slider, Skip ±10 s und Elapsed-Anzeige sind entfernt. Der zentrale Atemkreis (280 × 280) zeigt drei Schichten: statischer Track, Restzeit-Bogen (waechst mit Fortschritt) und atmender Glow (~16 s Zyklus, kontinuierlich, unabhaengig vom Audio — auch bei Pause weiter). In der Hauptphase sitzt ein Glas-Pause-Button (`.ultraThinMaterial`) mittig im Glow; bei Pre-Roll laeuft stattdessen der Countdown im Inneren mit dem Hint "GLEICH GEHT'S LOS" unter dem Ring. Restzeit-Label "NOCH 8:32 MIN" als ruhiges, sekundaeres Label unter dem Ring. Bei aktivem "Bewegung reduzieren" laeuft die Atem-Animation nicht; Restzeit-Bogen aktualisiert sich weiterhin (Information). Layout responsiv von iPhone SE bis Pro Max ohne Scrollen. (Ticket: shared-087)
