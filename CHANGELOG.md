...
### Changed (iOS)
- **Typografie aus dem Handover uebernommen — Display in Newsreader (Serif), UI in Geist (Sans)** - Die App spricht jetzt mit zwei Schrift-Familien statt einer einzigen System-Font: Display-Texte (Hero- und Section-Titel, Body, Timer-Ziffern, Dialog-Titel) tragen die Serif Newsreader (16pt-Optical-Size, Light/Regular/Medium), UI-Texte (Library- und Track-Listen-Titel, Settings-Labels, Eyebrows, CTA-Buttons, Dial-Einheit, Code) tragen die Sans Geist (Light/Regular/Medium). Die Halation-Kompensation fuer Dark Mode greift weiter: Weights eine Stufe schwerer pro Stufe (ultraLight→thin, thin→light, light→regular, regular→medium) — die System-Weights werden anschliessend auf die naechste vorhandene Custom-Familie-Stufe geklemmt (300/400/500). Beide Familien sind aus Privacy-Gruenden statisch ins App-Bundle eingebunden (kein Laufzeit-Download), gemeinsame OFL-1.1-Lizenz liegt mit im Bundle. Der dekorative `.italic()`-Modifier auf dem "von X Minuten"-Label im Running-Timer ist raus, weil Italic nach Handoff nur fuer Hervorhebungen in Akzentfarbe vorgesehen ist. Dynamic Type skaliert weiterhin korrekt — Custom Fonts werden ueber `Font.custom(_:size:relativeTo:)` an die TextStyle-Skalierung gebunden. (Ticket: ios-048)
